const {findAll, findOneUser, modifyOneUser, addUser, removeUser, findByMail} = require("./model");
...
const register = async (req, res) => {
  ...
  try {
    const result = await findByMail(user.email);
    if (result.length > 0) {
      res.status(409).send({
...
const login = async (req, res) => {
  ...
  try {
    ...
    if (userLogin.length === 0) {
      ...
      const { id, email, isAdmin } = userLogin[0];
